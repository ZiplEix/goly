...
func getAllGolies(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"massage": "error getting all goly links " + err.Error(),
		})
	}
...
func getGoly(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON((fiber.Map{
			"massage": "error could not parse id " + err.Error(),
		}))
	}
...
func createGoly(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON((fiber.Map{
			"massage": "error parsing JSON " + err.Error(),
		}))
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON((fiber.Map{
			"massage": "could not creat goly in db " + err.Error(),
		}))
	}
	return c.Status(fiber.StatusOK).JSON(goly)
}
